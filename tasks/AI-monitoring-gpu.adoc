...
----
+
IMPORTANT: Verify that the `ServiceAccount` name and namespace in the `RoleBinding` match your {otelemetry} Collector's deployment. 
+
...
----
{prompt.user}kubectl apply -n gpu-operator -f otel-rbac.yaml 
----
