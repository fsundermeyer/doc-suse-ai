...
. Update {ingress} configuration by creating the [path]``ingress.yaml`` file with the following content. 
+
...
apiVersion: networking.k8s.io/v1
  ...
  metadata:
    name: open-webui
    namespace:`SUSE_AI_NAMESPACE`annotations:
      nginx.ingress.kubernetes.io/proxy-body-size: 100m <1>nginx.ingress.kubernetes.io/proxy-connect-timeout: "360"
...
+
<1>
      This example sets the upload file size limit to 100{nbsp}MB. Setting this
      value to `
0`
 allows unlimited upload size (not
      recommended).
...
----
{prompt.user}kubectl apply -f ingress.yaml 
----
...
WARNING: Before {owui} starts to synchronize the directory, all existing resources in the collection are deleted. 
+
...
WARNING: If you experience the error saying "`Extracted content is
                not available for this file. Please ensure that the file is
                processed before proceeding,`"
 during the upload process, you need to enable installing NLTK datasets in the {owui} {helm} chart.
Refer to link:{dsc}/suse-ai/1.0/html/AI-deployment-intro/index.html#owui-helm-overrides for details. 
+
...
.Viewing an extracted resource [[_owui_knowledgebase_viewtext]]
